...
*/
		...
		for (int i=0; i < 9; i++){
			r1.setMenoresImagens(r1.getImagens(i));
		}
...
/**/		float max = -100;
			...
			for (int k=0; k < 9; k++) {
				if (menoresImagens.get(i).getSaldoDistance() > max) {
					max = menoresImagens.get(i).getSaldoDistance();
						index = i;
				}
			
/**/		}
			
			if (r1.getImagens(i).getSaldoDistance() < r1.getMenoresImagens(index).getSaldoDistance() ) {
					r1.setTroca(index,r1.getImagens(i));//menoresImagens.set(k, r1.getImagens(i));
			}
		}
		int cont1 = 0;
		int cont2 = 0;
		for (int i=0; i < 9; i++) {
			if (r1.getMenoresImagens(i).getTipo().equals("noPerson")) {
				cont2 ++;
			} else {
				cont1++;
			}
	...
	public Caracteristica getMenoresImagens(int i) {
		return menoresImagens.get(i);
	}

	public void setMenoresImagens(Caracteristica menorI) {
		Caracteristica c1 = new Caracteristica();
		c1 = menorI;
		this.menoresImagens.add(c1);
	}
	public void setTroca (int k, Caracteristica c) {
		menoresImagens.set(k,c );
	}
